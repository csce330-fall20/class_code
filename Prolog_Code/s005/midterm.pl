...
% employee(entity_id, job_title, name, employee id, employment_start_year, emplyoment_end_year)
...
mayor_of_yemassee(Name) :- 
    entity(ID,city,yemassee),
    employee(ID,mayor,Name,_,_,-1).
...
current_governor_past_jobs(Job_Title) :- 
    entity(StateID,state,sc),
    employee(StateID,governor,_,EID,_,-1),
    employee(_,    Job_Title ,_,EID,_,End),
    End > -1.
...
two_jobs_two_places(Name, EmpID) :- 
    employee(ID1,_,Name,EmpID,_,-1),
    employee(ID2,_,Name,EmpID,_,-1),
    \+ ID1 = ID2.
...
binary_tree_search(N) :- root(Current), bts_helper(N,Current).
...
%DOWN the tree
path_to_node(N,Path):-root(R),node(N),ptn(N,R,Path).

ptn(N,N,[N]).
ptn(N,C, [C|PtN] ) :- N<C, node(C,L,_), \+L=nil, ptn(N,L,PtN) .
ptn(N,C, [C|PtN] ) :- N>C, node(C,_,R), \+R=nil, ptn(N,R,PtN) .

%UP the tree
path_to_node1(R,[R]) :- root(R).
path_to_node1(N, Path2N) :-
