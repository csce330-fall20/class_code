...
binary_tree_search(N) :- root(R), node(N), bts(N,R).

%bts(Node,Current)
bts(C,C).
bts(N,C):-N<C,node(C,L,_),bts(N,L).
bts(N,C):-N>C,node(C,_,R),bts(N,R).
...
ptn(N,N,[N]):-node(N).
ptn(N,C,[C|PtN]):-node(C,L,_),ptn(N,L,PtN).
ptn(N,C,[C|PtN]):-node(C,_,R),ptn(N,R,PtN).

%up
path_to_node1(R,[R]):-root(R).
path_to_node1(N,Path):-
    (node(P,N,_); node(P,_,N)),
    node(N),
    path_to_node1(P,PPath),
    append(PPath,[N],Path).
