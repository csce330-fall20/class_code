...
quotient_remainder(X,Y,Q,R) :- Q is X div Y, R is mod(X,Y).

%quotient_remainder2(X,Y,Q,R) :- 

%square function - B is input
square(B,S) :- S is B*B.

%sqrt(N). Kind of pointless. S is input
...
power( _ ,0,1).
power(A,P,A2P) :- P>0,Pm1 is P-1, power(A,Pm1,A2Pm1), A2P is A*A2Pm1.
power(A,NP,A2NP) :- NP<0, P is -1*NP, power(A,P,A2P), A2NP is 1/A2P.
...
above1(Bt,Bb):-loc(Bt,X,Yt), loc(Bb,X,Yb), Yb - Yt >0.
